...
function SnowStorm() {
   ...
   var usePNG = true;
    var imagePath = 'http://event.icantw.com/hy/test/images/fire/'; // relative path to snow images
    var flakeTypes = 3;/*圖片數量*/
    var flakesMax = 6;/*畫面最多數量*/
    var flakesMaxActive = 6;/*畫面出現數量*/
    var vMax =5;/*速度*/
    var flakeWidth = 70;/*圖片大小*/
    var flakeHeight = 70;
    ...
    this.SnowFlake = function(parent, type, x, y) {
        ...
        this.step = 0;
        this.vHbTypes = [0, 1, 1, 1, 2, 3, 4, 0, 6, -1, -1, -1, -2, -3, -4, 0, -6];
        this.vHb = this.vHbTypes[Math.round(rnd(16))];
        this.vAlpha = 50;
        ...
        this.refresh = function() {
            ...
            //	  this.o.filters.alpha.opacity = this.vAlpha;
        }
        ...
        this.move = function() {
            this.step = this.degree += this.vHb;
            this.vX = vMax * Math.sin(this.step * Math.PI / 180);
            this.vY = (vMax * 2) * Math.cos(this.step * Math.PI / 180);
            this.x += this.vX;
            this.y += this.vY;
            this.vHb = this.vHbTypes[Math.round(rnd(16))];
            this.refresh();
            ...
            var yDiff = screenY + scrollY - this.y - storm.terrain[Math.floor(this.x)] - flakeHeight;
            if (yDiff < 5) {
                this.o.style.display = 'none';
                this.active = 0;
                this.degree = Math.round(rnd(45) - 152.5);
            } else if (this.y < flakeHeight / 2) {
                this.y = rnd(screenY);
                this.x = rnd(screenX);
                this.degree = Math.round(rnd(45, 67.5));
            }
        }
    ...
    this.createSnow = function(limit, allowInactive) {
        if (showStatus) window.status = 'Creating firefly...';
...
function PNGHandler() {
    ...
    this.filterMethod = function(o) {
        ...
        if (o.nodeName != 'IMG') {
            ...
            // apply filter
            o.src = 'http://www.furu2.jp/filter/images/none.gif'; // get rid of image
            o.style.filter = "progid:" + self.filterID + "(src='" + newSrc + "',sizingMethod=" + (o.className.indexOf('scale') + 1 ? 'scale' : 'crop') + "')";
            o.style.writingMode = 'lr-tb'; // Has to be applied so filter "has layout" and is displayed. Seriously. Refer to http://msdn.microsoft.com/workshop/author/filter/reference/filters/alphaimageloader.asp?frame=true
        }
    }
...
addEventHandler(window, 'load', snowStormInit, true);